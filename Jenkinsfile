pipeline {
  ...
  stages {
    stage('Check Python Version') {
      steps {
        bat '"C:\\Users\\braya\\AppData\\Local\\Microsoft\\WindowsApps\\python.exe" --version'
      }
    }
    ...
    stage('Instalar dependencias') {
      steps {
        bat """
          "C:\\Users\\braya\\AppData\\Local\\Microsoft\\WindowsApps\\python.exe" -m venv %VENV%
    ...
    stage('Construir imagen Docker') {
      steps {
        script {
          dockerImage = docker.build("bacv04/clase10-taller")
        }
      }
    }
    ...
    stage('Push to DockerHub') {
      steps {
        script {
          docker.image('bacv04/clase10-taller').push()
        }
      }
    }
